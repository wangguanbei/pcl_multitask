...
public:
    // hyper parameters ()
    const double LIDAR_HEIGHT = -0.1; // lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord) / if you use robot-coord-transformed lidar scans, just set this as 0.

    const int    PC_NUM_RING = 20; // 20 in the original paper (IROS 18)
    const int    PC_NUM_SECTOR = 60; // 60 in the original paper (IROS 18)
    const double PC_MAX_RADIUS = 5.0; // 80 meter max in the original paper (IROS 18)
    ...
    const int    NUM_CANDIDATES_FROM_TREE = 50; // 10 is enough. (refer the IROS 18 paper)
    ...
    double SC_DIST_THRES = 0.45; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold / if not, recommend 0.1-0.15
